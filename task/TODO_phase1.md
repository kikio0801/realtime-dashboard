...
- [O] QR 로그인 동적 세션(매번 바뀌는 QR)으로 인한 중복 가입 문제 해결
  - [O] **이슈:** QR 코드가 매번 새로 발급되므로, 브라우저 세션이 날아간 상태(기기 변경, 시크릿 모드 등)에서 로그인하면 무조건 '신규 의료진'으로 DB에 생성되는 한계 존재.
  - [O] **해결안 1:** 닉네임과 함께 **전화번호(Phone Number)**를 추가 식별 정보로 입력받아 기존 `medical_staff` 레코드와 매칭(연동)하는 로직 구현. (이름 + 전화번호를 고유 키로 활용)
  - [O] **해결안 2 (Edge Case):** 이름과 전화번호가 우연히 완전히 일치하는 극단적 케이스 처리 정책 적용 (단일 식별자로 취급하여 하나로 병합 처리).
