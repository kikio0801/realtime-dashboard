...
- [ ] **Supabase 본격 연동 및 데이터베이스 구축**
  - [ ] 발급된 PAT(Personal Access Token)를 활용하여 프로젝트 환경 구성
...
- [ ] 프론트엔드 헤더 프로필의 '로그아웃' 버튼 동작 오류 수정 및 연동
- [ ] QR 로그인 동적 세션(매번 바뀌는 QR)으로 인한 중복 가입 문제 해결
  - [ ] **이슈:** QR 코드가 매번 새로 발급되므로, 브라우저 세션이 날아간 상태(기기 변경, 시크릿 모드 등)에서 로그인하면 무조건 '신규 의료진'으로 DB에 생성되는 한계 존재.
  - [ ] **해결안 1:** 닉네임과 함께 **전화번호(Phone Number)** 등 추가 식별 정보를 입력받아 기존 `medical_staff` 레코드와 매칭(연동)하는 기능 추가.
  - [ ] **해결안 2 (Edge Case):** 포트폴리오용이므로 실제 본인 인증(KCB 등)은 생략하되, 이름과 전화번호가 우연히 완전히 일치하는 극단적 케이스에서 어떻게 기존 계정에 덮어씌울지/보호할지 정책적/화면적 고민 필요.
- [ ] QR 로그인 화면 또는 마이페이지에서 `department`(부서) 및 `email` 정보 추가 입력받기 (로그인 시 빈 값(NULL)으로 남는 문제 해결 및 데이터 정합성 확보)
