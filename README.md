...
- **환자 중심 모니터링**: 병실 내 환자들의 상태를 실시간으로 파악하여 담당 간호사의 효율적인 관리를 지원합니다.
- **업무 효율 극대화**: 빠르고 안정적인 데이터 통신과 직관적인 UI/UX를 통해 의료 현장의 피로도를 낮춥니다.
- **데이터 흐름 검증**: 단순 기능 구현을 넘어 Next.js - FastAPI - Pandas 간의 유기적인 데이터 흐름을 실험하고 검증합니다.
- **시스템 통합 사례**: 헬스케어 데이터의 특성을 반영하여 Supabase와 Pandas를 연동한 최적의 데이터 파이프라인을 구축했습니다.
...
### 💡 모바일 접속 관련 참고
1. 현재 프로젝트는 로컬 서버에서 실행되므로, **PC와 모바일 기기가 서로 같은 Wi-Fi 환경에 연결되어 있어야만** 접속이 가능합니다.
2. **공공장소 네트워크**에서는 보안상 기기 간 통신이 차단되어 QR 로그인이 작동하지 않을 수 있습니다. 
3. 이 경우 **모바일 핫스팟** 사용을 권장하며, 연결이 불안정할 경우 PC 브라우저에서 직접 테스트해 주세요.
