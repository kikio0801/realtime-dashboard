...
- **QR 로그인 (Concept)**:
  ...
  - **스마트 세션 연동 (Session Synchronization)**: 동적 QR 코드의 특성상 접속 시마다 QR 해시값이 매번 달라집니다. 시스템은 이를 보완하기 위해 **이름과 전화번호를 고유 식별자로 지정**합니다. 다중 기기 또는 기존 사용자 접속 시, 이 식별자가 일치하면 새로운 로그인 인증 키(`auth_id`)를 기존 데이터에 덮어씌워 매끄럽게 연동합니다. 이 방식은 기존에 접속되어 있던 다른 기기의 데이터 접근 권한을 즉시 무효화하여 간편 로그인을 사용하면서도 **의료 데이터 유출을 방지하는 강력한 보안 효과**를 발휘하며, 과거 담당 환자 기록을 완벽하게 유지합니다.
